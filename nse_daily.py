#!/usr/bin/env python3
"""
NSE Smart Money — Daily Report

HOW IT WORKS:
- Reads bulk_1y.csv + block_1y.csv (1Y historical files YOU download once from NSE)
- Fetches today's fresh deals from NSE API (small request = not blocked)
- Merges everything, builds interactive heatmap, emails as HTML attachment
- Runs automatically every weekday at 7am via Mac launchd scheduler
"""

import os, sys, time, json, smtplib, warnings, io
import requests
import pandas as pd
...
def fetch_today(session, deal_type, today_str):
    url = f"https://www.nseindia.com/api/historical/{deal_type}"
    try:
        r = session.get(url, params={"from": today_str, "to": today_str}, timeout=20)
        if r.status_code == 200:
            d = r.json()
            data = d.get("data", d) if isinstance(d, dict) else d
            if isinstance(data, list):
                print(f"  Today {deal_type}: {len(data)} rows")
                return data
    except Exception as e:
        print(f"  Today {deal_type} failed: {e}")
    return []


def parse_nse_csv(filepath, source):
    path = Path(filepath)
    if not path.exists():
    ...
    print(f"  {source} raw: {len(df)} rows, cols: {list(df.columns[:6])}")
    ...
    col_map = {
        ...
        "weighted_average_price":"price", "wght_avg_price":"price",
...
def parse_raw_api(rows, source):
    ...
    for r in rows:
        def g(*keys):
            for k in keys:
                v = r.get(k) or r.get(k.upper()) or r.get(k.lower())
                if v not in (None, "", "N/A", "-"): return str(v).strip()
            return ""
        sym   = g("SYMBOL","symbol","Symbol").upper()
        side  = g("BUY_SELL","buySell","Buy / Sell").upper()
        qty   = clean_num(g("QTY_TRD","quantity","Quantity Traded"))
        price = clean_num(g("TRADE_PRICE","tradePrice","Trade Price / Wght. Avg. Price"))
        cli   = g("CLIENT_NAME","clientName","Client Name")
        sec   = g("SCRIP_NAME","securityName","Security Name")
        dstr  = g("DT_DATE","date","Date")
...
def summarise(sub):
    ...
    for sym, grp in sub.groupby("symbol"):
        ...
        tb_cr = round(float(buy_g.groupby("client")["val_cr"].sum().max()), 2) if not buy_g.empty else 0
...
def fetch_live_mcaps(session, syms, known):
    mcap    = dict(known)
    missing = [s for s in syms if not mcap.get(s)]
    print(f"  Fetching live market caps for {len(missing)} symbols...")
    for sym in missing:
        try:
            r = session.get(f"https://www.nseindia.com/api/quote-equity?symbol={sym}", timeout=8)
            d = r.json()
            mc = d.get("marketDeptOrderBook",{}).get("tradeInfo",{}).get("totalMarketCap",0)
            if mc: mcap[sym] = round(float(mc)/1e7)
        except: pass
        time.sleep(0.2)
    loaded = sum(1 for v in mcap.values() if v > 0)
    print(f"  Market caps: {loaded}/{len(syms)} available")
    return mcap
...
async function fetchMissingMcaps(){{const pill=document.getElementById('mcap-pill');const allSyms=[...new Set([...D.buy_syms,...D.sell_syms,...D.buy_syms_7d,...D.sell_syms_7d])];const missing=allSyms.filter(s=>!MCAP[s]||MCAP[s]===0);let loaded=Object.values(MCAP).filter(v=>v>0).length;const total=allSyms.length;if(missing.length===0){{pill.className='mcap-pill done';pill.textContent='\u2713 Market caps: '+loaded+'/'+total;return;}}pill.className='mcap-pill loading';pill.textContent='\u29d6 Fetching '+missing.length+' caps...';async function tryY(sym){{try{{const r=await fetch('https://query1.finance.yahoo.com/v8/finance/chart/'+sym+'.NS?interval=1d&range=1d');if(!r.ok)throw 0;const d=await r.json();const mc=(((d||{{}}).chart||{{}}).result||[{{}}])[0]?.meta?.marketCap||0;return Math.round(mc/1e7);}}catch{{return 0;}}}}for(let i=0;i<missing.length;i+=5){{const batch=missing.slice(i,i+5);await Promise.all(batch.map(async sym=>{{const mc=await tryY(sym);if(mc>0){{MCAP[sym]=mc;loaded++;const el=document.getElementById('mc-'+sym);if(el)el.textContent=fMcap(sym);}}}})  );pill.textContent='\u29d6 '+loaded+'/'+total+' caps';await new Promise(res=>setTimeout(res,300));}}pill.className='mcap-pill done';pill.textContent='\u2713 Market caps: '+loaded+'/'+total;render('monthly',vs.monthly);render('daily',vs.daily);}}
render('monthly','buy');render('daily','buy');fetchMissingMcaps();"""

    return f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NSE Smart Money &mdash; {run_date}</title><style>{css}</style></head><body><div class="wrap"><div class="hdr"><div><h1>&#9889; NSE Smart Money Flow</h1><p>Bulk + Block Deals &nbsp;&middot;&nbsp; {run_date} &nbsp;&middot;&nbsp; Top {TOP_N} per view &nbsp;&middot;&nbsp; Buy/sell avg price &middot; % of market cap</p></div><div id="mcap-pill" class="mcap-pill">&#8987; Loading&hellip;</div></div><div class="tabs"><button class="tab on" onclick="sw('monthly',this)">&#128197; Monthly heatmap (1Y)</button><button class="tab" onclick="sw('daily',this)">&#128198; 7-day heatmap</button></div><div id="tab-monthly"><div class="controls"><button class="tgl on-buy" id="tm-buy" onclick="setView('monthly','buy')">&#8679; Top {TOP_N} net buyers</button><button class="tgl" id="tm-sell" onclick="setView('monthly','sell')">&#8681; Top {TOP_N} net sellers</button><span class="info">Net (Cr) &middot; Weighted avg price &middot; % of market cap</span></div><div class="hm-outer" id="monthly-wrap"></div></div><div id="tab-daily" style="display:none"><div class="controls"><button class="tgl on-buy" id="td-buy" onclick="setView('daily','buy')">&#8679; Top {TOP_N} net buyers</button><button class="tgl" id="td-sell" onclick="setView('daily','sell')">&#8681; Top {TOP_N} net sellers</button><span class="info">Net (Cr) &middot; Weighted avg price &middot; % of market cap</span></div><div class="hm-outer" id="daily-wrap"></div></div></div><script>{js}</script></body></html>"""
...
def send_email(html_path, run_date):
    ...
    msg.attach(MIMEText(
        f"NSE Smart Money Flow \u2014 {run_date}\n\nInteractive heatmap attached.\nDownload and open in Chrome/Safari.",
        "plain"
    ))
    ...
    print(f"  \u2705 Email sent ({Path(html_path).stat().st_size//1024}KB attachment)")
...
def main():
    ...
    print(f"\nNSE Smart Money \u2014 {run_date}")

    print("\nLoading historical CSV files...")
    bulk_1y  = parse_nse_csv(SCRIPT_DIR / "bulk_1y.csv",  "BULK")
    block_1y = parse_nse_csv(SCRIPT_DIR / "block_1y.csv", "BLOCK")

    if bulk_1y.empty and block_1y.empty:
        print("\n\u274c ERROR: No CSV files found in", SCRIPT_DIR)
        sys.exit(1)

    print("\nFetching today's deals from NSE...")
    session   = nse_session()
    today_str = today.strftime("%d-%m-%Y")
    bulk_today  = fetch_today(session, "bulk-deals",  today_str)
    block_today = fetch_today(session, "block-deals", today_str)
    df_today = pd.concat([
        parse_raw_api(bulk_today,  "BULK"),
        parse_raw_api(block_today, "BLOCK")
    ], ignore_index=True)
    print(f"  Today: {len(df_today)} rows")

    df = pd.concat([bulk_1y, block_1y, df_today], ignore_index=True)
    ...
    mcap = fetch_live_mcaps(session, all_syms, KNOWN_MCAP)
